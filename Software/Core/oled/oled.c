...
void OLED_Clear(void)
{
	...
	for(i=0;i<8;i++)
	{
		...
		OLED_WR_Byte (0x00,OLED_CMD);//OLED_WR_Byte (0x02,OLED_CMD);      //设置起始列低地址
...
{
	OLED_DrawBMP(0,0,32,4,Num_Set[calendar.hour/10]);
	OLED_DrawBMP(32,0,64,4,Num_Set[calendar.hour%10]);
	OLED_DrawBMP(64,3,96,7,Num_Set[calendar.min/10]);
	OLED_DrawBMP(96,3,128,7,Num_Set[calendar.min%10]);
}
